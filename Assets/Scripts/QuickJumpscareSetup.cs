...
public class QuickJumpscareSetup : MonoBehaviour
{
    [Header("Setup")]
    [Tooltip("Click this button in Inspector to auto-create jumpscare UI")]
    ...
    public void SetupJumpscareUI()
    {
        ...
        if (canvas == null)
        {
            Debug.LogError("<color=red>[Jumpscare Setup]</color> No Canvas found!");
            return;
        }

        if (FindObjectOfType<JumpscareSystem>() != null)
        {
            Debug.LogWarning("<color=yellow>[Jumpscare Setup]</color> JumpscareSystem already exists!");
            return;
        }
        ...
        Debug.Log("<color=green>[Jumpscare Setup]</color> Jumpscare System created successfully!");
